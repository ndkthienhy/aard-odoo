...
class HrLunch(models.Model):   
    ...
    total_working_hours = fields.Float('Total Working Hours', compute='_compute_total_wk')
    total_overtime_hours = fields.Float('Total Overtime Hours', compute='_compute_total_ot')
    total_meal = fields.Integer('Subtotal Meal', compute='_compute_total_meal', readonly=False, store=False)
    lunch_rule_id = fields.Many2one('aard.hr.lunch.rule', compute='_compute_lunch_rule_id', string='Lunch Rule', auto_join=True)
    ...
    def _compute_total_meal(self): 
         for record in self:
            if (record.lunch_rule_id or record.lunch_overtime_rule_id) and (record.total_working_hours > 0 or record.total_overtime_hours > 0):
                ...
                record.total_meal = number_meal * record.lunch_rule_id.number_meal + number_ot_meal * record.lunch_overtime_rule_id.number_meal
...
class HrLunchAttendance(models.Model):
    ...
    # @api.multi
    # def _onchange_valid_worked_hours(self):
    #     self.ensure_one()

    #     if self.id:
    #         lunchs = self.env['aard.hr.lunch'].search([('attendance_id', '=', self.id)])
            
    #         if lunchs:
    #             for rec in lunchs:
    #                 rec.write({'total_working_hours': self.valid_worked_hours})